...
public class ContractLeaseCommon implements ViewPagersInterface<Contract> {
	
	// 请求码定义
	public static final int USER_SELECT_REQUEST_CODE = 2000;
	public static final int CONTACT_COMPANY_SELECT_REQUEST_CODE = 2011;
	 ...
	 * @param isAddMode
	 */
	public ContractLeaseCommon(Context context, CallBack<Void, ContractType> callBack, 
							boolean isAddMode) {
		mContext = context;
		mCallBack = callBack;
	...
	public Contract updateContract(Contract contract) {
		...
		// 合同类型
		contract.setType(Integer.parseInt(GLOBAL.CONTRACT_TYPE[2][0]));
		contract.setCode(saveMap.get(mContext.getString(R.string.contract_code)));
		...
		if (!saveMap.get(mContext.getString(R.string.supplier)).isEmpty() 
			...
			contract.setSecond_party_name(saveMap.get(mContext.getString(R.string.supplier)));
		}
		
		contract.setName(saveMap.get(mContext.getString(R.string.contract_name)));
		contract.setSecond_party_address(saveMap.get(mContext.getString(R.string.company_address)));
