...
public class ContractAttachment extends ListWithOptionMenu<Files> 
									implements ViewPagersInterface<Contract> {
	
	// 文件选择请求码
	public static final int FILE_SELECT_REQEUST_CODE = 6000;
	
	// 该类索引，标识该类的回调调用
	public static final int CONTRACT_ATTACHMENT_INDEX = 4100;
	 ...
	 * @param hasSildePanel
	 */
	public ContractAttachment(Context context, boolean isIncomeContract,
			boolean isModify, Boolean isAddMode, boolean hasSildePanel) {
		super(context);
		mIsAddMode = isAddMode;
		commonInit(isModify, isIncomeContract, serviceInterface, hasSildePanel);
	}
	 ...
	 * @param hasSildePanel
	 */
	public ContractAttachment(Context context,
			boolean isIncomeContract,
			ServiceInterface<Files> serviceInterface, 
			boolean hasSildePanel) {
		super(context);
		mIsAddMode = true;
		commonInit(true, isIncomeContract, serviceInterface, hasSildePanel);
	}
		...
		if (isModify) {
			...
			mFloatingMenu.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (mContract != null) {
						files.setType_id(mContract.getContract_id());
					}
						...
						case 1:		// 查看
							if (mIsIncomeContract) {
								IntentBuilder.view(mContext, getCurrentItem(), 
											GLOBAL.INCOME_CONTRACT_DOWNLOAD_FILE_DIR);
							} else {
								IntentBuilder.view(mContext, getCurrentItem(), 
											GLOBAL.OUTCOME_CONTRACT_DOWNLOAD_FILE_DIR);
							}
							break;
						case 2:
							commonConfirmDelete();
			...
			return new SubMenuListener() {
				...
				public void onSubMenuClick(View view) {
					...
					switch ((Integer) view.getTag()) {
						...
						case 1:	// 查看
							if (mIsIncomeContract) {
								IntentBuilder.viewFile(mContext, getCurrentItem(), 
											GLOBAL.INCOME_CONTRACT_DOWNLOAD_FILE_DIR);
							} else {
								IntentBuilder.viewFile(mContext, getCurrentItem(), 
											GLOBAL.OUTCOME_CONTRACT_DOWNLOAD_FILE_DIR);
	...
	protected ServiceInterface<Files> serviceInterface 
		...
		public void getListData() {
			...
			if (mContract.getAttachments() != null 
						&& !mContract.getAttachments().isEmpty()) {
				attachment = mContract.getAttachments();
			}
			
			if (mIsIncomeContract) {
				mRevenueContractService.getContractFiles(getServiceManager(), 
							mContract.getContract_id(), attachment);
			} else {
				mExpensesContractService.getContractFiles(getServiceManager(), 
						mContract.getContract_id(), attachment);
			}
		}
		...
		public void addItem(Files t) {
			...
			t.setType_id(mContract.getContract_id());
			
			if (mIsIncomeContract) {
				t.setDir_type(Integer.parseInt(GLOBAL.FILE_TYPE[3][0]));
			} else {
				t.setDir_type(Integer.parseInt(GLOBAL.FILE_TYPE[4][0]));
			}
		...
		public void updateItem(Files t) {
			mDocumentsService.updateFile(getServiceManager(), t, new ArrayList<User>());
		}
	};
	...
	protected void floatingMenuItemClick(AdapterView<?> parent, View view,
		...
		switch (position) {
			case 0:
				...
				intent.putExtra("is_upload", !mIsAddMode);
	...
	public void handleParentEvent(Contract b) {
		if (mContext instanceof CallBack) {
			((CallBack<Void, Integer>) mContext).callBack(CONTRACT_ATTACHMENT_INDEX);
		}
	...
	public void handleDataOnResult(ResultStatus status, List<?> list) {
		...
		switch (status.getCode()) {
			case AnalysisManager.SUCCESS_DB_ADD:
				...
				String attachmentString = mContract.getAttachments();
				...
				mContract.setAttachments(attachments.toString());
				
				if (--mAddCount == 0) {
					mHasModify = true;
					if (mIsIncomeContract) {
						mRevenueContractService.updateRevenueContract(contractUpdateInterface,
								mContract);
					} else {
						mExpensesContractService.updateExpensesContract(contractUpdateInterface, 
								mContract);
					}
				}
				break;
			case AnalysisManager.SUCCESS_DB_DEL:
				String attachment = mContract.getAttachments();
				LogUtil.e("attachment = " + attachment + " mCurrentItem.getFile_id() = "  + mCurrentItem.getFile_id());
				...
				if (!sb.toString().isEmpty()) {
					...
					mContract.setAttachments(string);
				} else {
					mContract.setAttachments(sb.toString());
				}
