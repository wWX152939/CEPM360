...
public class CoverFlowView<T extends CoverFlowAdapter> extends View {
    ...
    private void imageOnTop(int position) {
        ...
        // replace this with your view size in order to get correctly calculated
        // rectangle
        // heightInView = 652;
        // widthInView = 1080 / 2;
        ...
        if (mTouchMoved || (mOffset - Math.floor(mOffset)) != 0) {
            ...
            if (mTouchRect != null) {
                if (mTouchRect.contains(event.getX(), event.getY())
                        ...
                        && !mLongClickTriggled) { // 点击中间的图片
                    final int actuallyPosition = mTopImageIndex;
                    mCoverFlowListener.topImageClicked(this, actuallyPosition);
                } else { // 点击左边 或 右边的图片
                	if (event.getX() < mTouchRect.left) {//点击左边 
                		if (mTopImageIndex == 0) {
                			setSelection(mAdapter.getCount() - 1, false); // 无动画
                		} else {
                			setSelection((mTopImageIndex - 1), true);
                		}
                	}
                	if (event.getX() > mTouchRect.right) { //点击右边                		
                		if (mTopImageIndex == mAdapter.getCount() - 1) {
                			setSelection(0, false); // 无动画
                		} else {
                			setSelection((mTopImageIndex + 1) % mAdapter.getCount(), true);
                		}
                	}
                }
            }
    ...
    private Bitmap obtainReflection(int position, Bitmap src) {
        ...
        if (reflection == null || reflection.isRecycled()) {
            ...
            reflection = createReflectedBitmap(src, reflectHeightFraction);
    ...
    private Bitmap createReflectedBitmap(Bitmap srcBitmap, float reflectHeight) {
        if (null == srcBitmap) {
            return null;
        }
     ...
     */
    public void setSelection(int position, boolean animation) {
        ...
        //if (mTopImageIndex != position) {
			...
			mScroller.startScroll(
					...
					!animation ? 0 : DURATION
							* Math.min(Math.abs(position + max - mTopImageIndex),
